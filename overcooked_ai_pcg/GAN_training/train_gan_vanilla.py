...
def run(
    ...
    for epoch in range(niter):
        ...
        print(
            '[%d/%d] Loss_G: %f Loss_D: %f Loss_D_real: %f Loss_D_fake %f D(x) %f D(G(z)) %f / %f'
            % (epoch, niter, average_errG, average_errD, average_errD_real,
               average_errD_fake, average_D_x, average_D_G_z1, average_D_G_z2))
