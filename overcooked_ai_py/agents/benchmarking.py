...
class AgentEvaluator(object):
    ...
    def evaluate_optimal_pair(self,
                              num_games,
                              delivery_horizon=2,
                              display=False):
    ...
    @staticmethod
    def get_joint_traj_in_single_agent_stable_baselines_format(
            trajs, encoding_fn, save=False, filename=None):
        """
        This requires splitting each trajectory into two, one for each action in the
        joint action.
        """
        sb_traj_dict_keys = [
            "actions", "obs", "rewards", "episode_starts", "episode_returns"
        ]
        ...
        for traj_idx in range(len(trajs["ep_lengths"])):
            ...
            for agent_idx in range(2):
                ...
                # with shape (1, )
                processed_agent_actions = [[
                    Action.ACTION_TO_INDEX[j_a[agent_idx]]
                ] for j_a in trajs["ep_actions"][traj_idx]]
                sb_trajs_dict["actions"].extend(processed_agent_actions)

                agent_obs = [
                    both_agent_obs[agent_idx] for both_agent_obs in
                    trajs["metadatas"]["ep_obs_for_both_agents"][traj_idx]
                ]
                ...
                traj_starts = [
                    1 if i == 0 else 0
                    for i in range(trajs["ep_lengths"][traj_idx])
                ]
                sb_trajs_dict["episode_starts"].extend(traj_starts)
                sb_trajs_dict["episode_returns"].append(
                    trajs["ep_returns"][traj_idx])
    ...
    def save_traj_as_json(trajectory, filename):
        ...
        assert set(OvercookedEnv.DEFAULT_TRAJ_KEYS) == set(
            trajectory.keys()), "{} vs\n{}".format(
                OvercookedEnv.DEFAULT_TRAJ_KEYS, trajectory.keys())
    ...
    @staticmethod
    def interactive_from_traj(trajectories,
                              traj_idx=0,
                              nested_keys_to_print=[]):
        """
        ...
        for nested_k in nested_keys_to_print:
            ...
            assert np.array(inner_data).shape == np.array(
                states).shape, "{} vs {}".format(
                    np.array(inner_data).shape,
        ...
        def update(t=1.0):
            ...
            joint_action = joint_actions[traj_timestep -
                                         1] if traj_timestep > 0 else (
