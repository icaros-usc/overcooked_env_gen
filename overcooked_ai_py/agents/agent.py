...
class AgentGroup(object):
    ...
    def __init__(self, *agents, allow_duplicate_agents=False):
        ...
        if not all(a0 is not a1
                   for a0, a1 in itertools.combinations(agents, 2)):
...
class AgentPair(AgentGroup):
    ...
    def __init__(self, *agents, allow_duplicate_agents=False):
        super().__init__(*agents,
                         allow_duplicate_agents=allow_duplicate_agents)
        assert self.n == 2
        self.a0, self.a1 = self.agents

        if type(self.a0) is CoupledPlanningAgent and type(
                self.a1) is CoupledPlanningAgent:
            print(
...
class AgentFromPolicy(Agent):
    ...
    def actions(self, states, agent_indices):
        ...
        for action_probs in action_probs_n:
            ...
            actions_and_infos_n.append((action, {
...
class CoupledPlanningAgent(Agent):
    ...
    def __init__(self, mlp, delivery_horizon=2, heuristic=None):
        self.mlp = mlp
        self.mlp.failures = 0
        self.heuristic = heuristic if heuristic is not None else Heuristic(
            mlp.mp).simple_heuristic
    ...
    def action(self, state):
        ...
        return (joint_action_plan[0][self.agent_index],
                {}) if len(joint_action_plan) > 0 else (Action.STAY, {})
...
class EmbeddedPlanningAgent(Agent):
    ...
    def action(self, state):
        ...
        order_list = start_state.order_list if start_state.order_list is not None else [
            "any", "any"
        ]
...
class GreedyHumanModel(Agent):
    ...
    def ml_action(self, state):
        ...
        if not player.has_object():
            ...
            if curr_order == 'any':
                ready_soups = pot_states_dict['onion'][
                    'ready'] + pot_states_dict['tomato']['ready']
                cooking_soups = pot_states_dict['onion'][
