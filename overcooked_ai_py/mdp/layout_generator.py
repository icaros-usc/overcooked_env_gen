...
class LayoutGenerator(object):
    ...
    def padded_mdp(self, mdp, display=False):
        ...
        mdp_grid = self.padded_grid_to_layout_grid(padded_grid,
                                                   start_positions,
                                                   display=display)
        return OvercookedGridworld.from_grid(
            mdp_grid, base_layout_params=self.mdp_params)

    def make_disjoint_sets_layout(self,
                                  inner_shape,
                                  prop_empty,
                                  prop_features,
                                  display=True):
    ...
    def padded_grid_to_layout_grid(self,
                                   padded_grid,
                                   start_positions,
                                   display=False):
        if display:
    ...
    def dig_space_with_disjoint_sets(self, grid, prop_empty):
        dsets = DisjointSets([])
        while not (grid.proportion_empty() > prop_empty
                   and dsets.num_sets == 1):
...
class Grid(object):
    ...
    def is_valid_feature_location(self, location):
        ...
        if any([
                loc for loc in self.get_near_locations(location)
    ...
    def convert_to_string(self):
        ...
        assert np.array_equal(string_grid.T.shape,
                              self.shape), "{} vs {}".format(
...
class Fringe(object):
    ...
    def pop(self):
        ...
        choice_idx = np.random.choice(len(self.fringe_list),
                                      p=self.distribution)
